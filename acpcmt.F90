...
ASSOCIATE(LCVMICC=>YRPHY0%LCVMICC, GCVFNEP1=>YRPHY0%GCVFNEP1, &
 & RDTFAC=>YRPHY0%RDTFAC, GCVFNEP2=>YRPHY0%GCVFNEP2, GSHELL=>YRPHY0%GSHELL, &
 & FNEBC=>YRPHY0%FNEBC, GREDDRS=>YRPHY0%GREDDRS, GCVFNER=>YRPHY0%GCVFNER, &
 & LCVTADV=>YRPHY0%LCVTADV, GCVFNEO=>YRPHY0%GCVFNEO, LCVNHD=>YRPHY0%LCVNHD, &
 & NCVCLOS=>YRPHY0%NCVCLOS, THPCMT=>YRPHY0%THPCMT, &
 & TSPHY=>YRPHY2%TSPHY, &
 & LNEBNXR=>YRPHY%LNEBNXR, LEDKF=>YRPHY%LEDKF, LCVPPKF=>YRPHY%LCVPPKF, &
 & NCVQLI=>YRPHY0%NCVQLI)
...
DO JLEV = KTDIA-1, KLEV
  DO JLON = KIDIA, KFDIA
    ZDETR_U_RS(JLON,JLEV)=GREDDRS*ZDETR_U(JLON,JLEV)
    ZDETR_D_RS(JLON,JLEV)=GREDDRS*ZDETR_D(JLON,JLEV)
  ENDDO
ENDDO
