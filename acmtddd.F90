...
ASSOCIATE(GAMAP1=>YRPHY0%GAMAP1, TENTR=>YRPHY0%TENTR, GCVADS=>YRPHY0%GCVADS, &
 & GDDSDE=>YRPHY0%GDDSDE, RKDN=>YRPHY0%RKDN, TENTRX=>YRPHY0%TENTRX, &
 & TDDGP=>YRPHY0%TDDGP, &
 & NBITER=>YRPHY%NBITER, LNEIGE=>YRPHY%LNEIGE, GCVADET=>YRPHY0%GCVADET)
! -----------------------------------------------------------------------
...
ZBCC_CD_HALF(:,:)=0._JPRB
PBCC_CD_FULL(:,:)=0._JPRB
...
!            : WATER VAPOUR SPECIFIC HUMIDITY OF THE DOWNDRAFT.
  ...
  ZTD=PT(JLON,KTDIA)*(1.0_JPRB+(PQLIC(JLON,KTDIA)-RETV*(PQSAT(JLON,KTDIA)&
   ...
   &/(RV*PT(JLON,KTDIA))))
  ZTD=GCVADET*ZTD+(1.0_JPRB-GCVADET)*ZTN(JLON)
  IF (LNEIGE) THEN
  ...
  ZQW=FOQS(ZESP)
  ZQW=GCVADET*ZQW+(1.0_JPRB-GCVADET)*ZQN(JLON)
...
! ZCAPE      : CAPE DU NIVEAU.
  ...
  PEPSI(JLON,KTDIA)=TENTRX*PR(JLON,KTDIA)*PT(JLON,KTDIA)/PAPRSF(JLON,KTDIA)
...
!     NEW PARAMETERIZATION (KERSHAW & GREGORY).
    ...
    DO JTRA=1,KTRA
      PTRAM(JLON,JLEV,JTRA)=PTRAM(JLON,JLEV-1,JTRA)&
       ...
       &+TDDGP*(PTRA(JLON,JLEV,JTRA)-PTRA(JLON,JLEV-1,JTRA)))&
       &/(1.0_JPRB-PBET(JLON,JLEV))
    ENDDO
...
!     COME BACK TO THE TRUE LATENT HEAT.
    ...
    ZQW=FOQS(ZESP)
    ICDN=MAX(0,-ISIGN(1,-KNCDN(JLON,JLEV)))
    ZTD=ICDN*(GCVADET*ZTD+(1.0_JPRB-GCVADET)*ZTN(JLON))+(1-ICDN)*ZTN(JLON)
    ZQW=ICDN*(GCVADET*ZQW+(1.0_JPRB-GCVADET)*ZQN(JLON))+(1-ICDN)*ZQN(JLON)
    IF (LNEIGE) THEN
      ZDELTA=MAX(0.0_JPRB,SIGN(1.0_JPRB,RTT-ZTD))
    ELSE
      ZDELTA=0.0_JPRB
