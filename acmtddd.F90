...
INTEGER(KIND=JPIM), INTENT(IN) :: KLEVTEN
...
temp (INTEGER(KIND=JPIM), INIVBDC, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZBCC_CD_HALF, (KLON,0:KLEV))
temp (REAL(KIND=JPRB), ZFMOD, (KLON,0:KLEV))
temp (REAL(KIND=JPRB), ZDWNDP, (KLON,0:KLEV))
temp (REAL(KIND=JPRB), ZVVER, (KLON,0:KLEV))
REAL(KIND=JPRB) :: ZVVERFM1
REAL(KIND=JPRB) :: ZTN
REAL(KIND=JPRB) :: ZTB
REAL(KIND=JPRB) :: ZRVH
REAL(KIND=JPRB) :: ZRBH
...
!            : WATER VAPOUR SPECIFIC HUMIDITY OF THE DOWNDRAFT.
  ...
  ZQN=PQW(JLON,KTDIA)
  PQN2(JLON,KTDIA)=ZQN
  PSN2(JLON,KTDIA)=(RCPD+ZCPVMD*ZQN)*ZTN+PAPHIF(JLON,KTDIA)
  PTN2(JLON,KTDIA)=ZTN
  ...
  ZTD=PT(JLON,KTDIA)*(1.0_JPRB+(PQLIC(JLON,KTDIA)-RETV*(PQSAT(JLON,KTDIA)&
   ...
   &/(RV*PT(JLON,KTDIA))))
  ZTD=YRPHY0%GCVADET*ZTD+(1.0_JPRB-YRPHY0%GCVADET)*ZTN
  ...
  ZQW=FOQS(ZESP)
  ZQW=YRPHY0%GCVADET*ZQW+(1.0_JPRB-YRPHY0%GCVADET)*ZQN
...
!            : "ZQN" AT THE BOTTOM OF THE SLAB WITH ENTRAINMENT "IN".
    ...
    ZMIX=ZENTR*(PAPHIF(JLON,JLEV-1)-PAPHIF(JLON,JLEV))
    ZMIX=ZMIX/(1.0_JPRB+ZMIX)
    ZTB=ZTN+ZMIX*(PT(JLON,JLEV-1)-ZTN)
    ZQB=ZQN+ZMIX*(PQ(JLON,JLEV-1)-ZQN)
...
!            : FOR THE Q DEPENDENT CORRECTION OF ZRBH.
    ...
    ZRBB=-PALPH(JLON,JLEV-1)*(RD+ZRVMD*ZQB)
    ZRBH=(-PLNPR(JLON,JLEV)+PALPH(JLON,JLEV))*(RD+ZRVMD*ZQB)
    ZRVH=(-PLNPR(JLON,JLEV)+PALPH(JLON,JLEV))*RV
...
!     SNOW OPTION DEPENDENT CALCULATIONS.
    ...
    IF (YRPHY%LNEIGE) THEN
      ZDELTA=MAX(0.0_JPRB,SIGN(1.0_JPRB,RTT-ZTB))
    ELSE
      ZDELTA=0.0_JPRB
    ENDIF
...
!            : AS PZLH BUT FOR THE SPECIFIC HEAT CP.
    ...
    ZLH= FOLH (ZTB,ZDELTA)+ZRVH*ZTB
    ZCP=RCPD+ZCPVMD*ZQB+ZRBH
...
!     NEWTON LOOP.
    ...
    ZLH=ZLH+ZDCP*ZDELT
    ZDELQ=-((ZRBB+ZRBH)*ZTB+ZCP*ZDELT)/ZLH
    ZCP=ZCP+ZDCP*ZDELQ
    ZTN=ZTB+ZDELT
    ZQN=ZQB+ZDELQ
...
!     SNOW OPTION DEPENDENT COMPUTATIONS.
      ...
      IF (YRPHY%LNEIGE) THEN
        ZDELTA=MAX(0.0_JPRB,SIGN(1.0_JPRB,RTT-ZTN))
      ELSE
        ZDELTA=0.0_JPRB
      ENDIF
...
!     SATURATION CALCULATIONS USING DEFINED FUNCTIONS.
      ...
      ZQW= FOQS (ZESP)
      ZDQW= FODQS (ZQW,ZESP, FODLEW (ZTN,ZDELTA))

!     INCREMENTATIONS.
!     INCREMENTATIONS.

      ZDCP=ZRVH+ZCPVMW+ZDELTA*(ZCPVMS-ZCPVMW)
      ZDELQ=(ZQW-ZQN)*ZCP/(ZCP+ZLH*ZDQW)
      ZCP=ZCP+ZDCP*ZDELQ
      ZDELT=-ZDELQ*ZLH/ZCP
      ZLH=ZLH+ZDCP*ZDELT
      ZQN=ZQN+ZDELQ
...
!     ADIABATIQUE SECHE.
    ...
    ZCPS=RCPD*(1.0_JPRB-ZQB)+RCPV*ZQB
    ZTNSEC=(ZCPS-ZRBB)*ZTB/(ZCPS+ZRBH)
    ICDN=MAX(0,-ISIGN(1,-KNCDN(JLON,JLEV)))
    ZTN=ICDN*ZTN+(1-ICDN)*ZTNSEC
    ZQN=ICDN*ZQN+(1-ICDN)*ZQB
    ZBCC_CD_HALF(JLON,JLEV)=MAX(0._JPRB,ZQN-ZQB) &
...
!     DURANT LE PAS DE TEMPS ET TEST DE SON SIGNE.
    ...
    ZDWNDP(JLON,JLEV-1)=(ZVVERF-ZVVERFM1)/ZDELPF
...
!     ENTRAINEMENT-DETRAINEMENT ORGANISES.
    ...
    IKUO2=MAX(0,ISIGN(1,KLEVTEN-ILEVBDC))
...
!     COME BACK TO THE TRUE LATENT HEAT.
    ...
    ZQW=FOQS(ZESP)
    ICDN=MAX(0,-ISIGN(1,-KNCDN(JLON,JLEV)))
    ZTD=ICDN*(YRPHY0%GCVADET*ZTD+(1.0_JPRB-YRPHY0%GCVADET)*ZTN)+(1-ICDN)*ZTN
    ZQW=ICDN*(YRPHY0%GCVADET*ZQW+(1.0_JPRB-YRPHY0%GCVADET)*ZQN)+(1-ICDN)*ZQN
    IF (YRPHY%LNEIGE) THEN
      ZDELTA=MAX(0.0_JPRB,SIGN(1.0_JPRB,RTT-ZTD))
    ELSE
      ZDELTA=0.0_JPRB
    ...
    PQN2(JLON,JLEV)=ZQN
    PSN2(JLON,JLEV)=(RCPD+ZCPVMD*ZQN)*ZTN+PAPHIF(JLON,JLEV)
    PTN2(JLON,JLEV)=ZTN
...
!     THAT OF THE WET BULB CHARACTERISTICS OF THE ENVIRONMENT.
    ...
    ZTN=INUA*ZTN+(1-INUA)*PTW(JLON,JLEV)
    ZQN=INUA*ZQN+(1-INUA)*PQW(JLON,JLEV)
