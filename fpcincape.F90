...
INTEGER(KIND=JPIM)      :: IPREVIOUS_NULL_ZRT(KLON)
INTEGER(KIND=JPIM) :: JLEV,JLON,JIT,JETAPES
REAL(KIND=JPRB) :: ZDLOG(KLON),ZBUOY,ZTV1,ZTV2,ZQV2,ZRT(KLON),ZT2,ZDT(KLON),ZST,ZDELARG,ZL,ZCP, &
 & ZFDERQS,ZADD,ZDERL,ZRDLOG,ZZQV
REAL(KIND=JPRB) :: ZBUOYPREC(KLON),ZQS(KLON),ZQV(KLON), ZQV1(KLON), &
 & ZT(KLON), ZT1(KLON),ZQL(KLON),ZQI(KLON),ZSQV(KLON), &
 & ZSQL(KLON),ZSQI(KLON),ZTDEPART(KLON),ZLOG(KLON), &
 & ZPARRIVEE(KLON),ZPDEPART(KLON),ZQVDEPART(KLON),ZZT(KLON)  
REAL(KIND=JPRB) :: ZFDERFOLH0(KLON),ZDELARG0(KLON),ZL0(KLON),ZCP0(KLON)
REAL(KIND=JPRB) :: ZFDERQS0, ZDZ, ZZFOEW(KLON)
REAL(KIND=JPRB) :: ZMAXT,ZMINT,ZMINDERI,ZMINQ,ZMAXQ
REAL(KIND=JPRB) :: ZTIN(KLON,KLEV) 
REAL(KIND=JPRB) :: ZQVIN(KLON,KLEV) 
...
ZBUOYPREC(KIDIA:KFDIA)=0.0_JPRB
ZT (KIDIA:KFDIA)=ZTIN (KIDIA:KFDIA,KLEVST)
ZQV(KIDIA:KFDIA)=ZQVIN(KIDIA:KFDIA,KLEVST)
ZQL(KIDIA:KFDIA)=0.0_JPRB
ZQI(KIDIA:KFDIA)=0.0_JPRB
