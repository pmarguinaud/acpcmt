...
REAL (KIND=JPRB) ATTR_ARG, POINTER :: PALL (:,:,:,:)
REAL (KIND=JPRB), POINTER :: P (:,:,:)
INTEGER :: KLB (3), KUB (3), JLON, JLON1, IBL1R, IBL1, IGP, ISIZ, IDIM
...
ISIZ = 1
DO IDIM = 1, 3
  ISIZ = ISIZ * MAX (KUB (IDIM) - KLB (IDIM) + 1, 0)
ENDDO

IF (ISIZ == 0) THEN
  IF (IBL == 1) THEN
    ALLOCATE (PALL (KLON, 1, 1, ICOUNT1))
    PALL = 0
  ENDIF
  RETURN
ENDIF
...
IF (KFDIA == IFDIA (IBL) .AND. LDPRINT) THEN
  ...
  ZALL = PALL (:, IBL)
  CALL DIFF (CDNAME, ZALL, P, LLLPRINT)
  DEALLOCATE (ZALL)
ENDIF
