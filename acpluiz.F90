...
INTEGER(KIND=JPIM) :: IPSTPT_ZFLUXCOR
...
INTEGER(KIND=JPIM) :: IPSTPT_ZQ

INTEGER(KIND=JPIM) :: IPSTPT_ZT

INTEGER(KIND=JPIM) :: IPSTPT_ZQSATS

INTEGER(KIND=JPIM) :: IPSTPT_ZRMF

INTEGER(KIND=JPIM) :: IPSTPT_ZRHCRI
...
IF ( LDADJCLD ) THEN
  ...
  DO JLEV = KTDIA, KLEV
    DO JLON = KIDIA, KFDIA
      PSTACK (JLON, IPSTPT_ZT+JLEV-(1)) = PT(JLON,JLEV)+YRPHY2%TSPHY*PTENDH(JLON,&
      & JLEV)/PCP(JLON,JLEV)
      PSTACK (JLON, IPSTPT_ZQ+JLEV-(1)) = MAX( 0.0_JPRB,PQ(JLON,JLEV)+YRPHY2%&
      & TSPHY*PTENDQ(JLON,JLEV) )
    ENDDO
  ENDDO

  CALL ACNEBSM ( KIDIA,KFDIA,KLON,KTDIA,KLEV, PSTACK (1, IPSTPT_ZT),             &
  & PSTACK (1, IPSTPT_ZQ),PQL,PQI,PAPHI,PAPRSF,PCP,PR, PGM,PVETAF, PQCS,PNEBS,   &
  & PSTACK (1, IPSTPT_ZRHCRI),PSTACK (1, IPSTPT_ZRMF),PSTACK (1, IPSTPT_ZQSATS), &
...
ELSE
  ...
  DO JLEV = KTDIA, KLEV
    DO JLON = KIDIA, KFDIA
      PSTACK (JLON, IPSTPT_ZT+JLEV-(1)) = PT(JLON,JLEV)
      PSTACK (JLON, IPSTPT_ZQ+JLEV-(1)) = PQ(JLON,JLEV)
    ENDDO
  ENDDO
...
DO JLEV = KTDIA, KLEV
  DO JLON = KIDIA, KFDIA
    ZICE = FONICE(PSTACK (JLON, IPSTPT_ZT+JLEV-(1)))
    ...
    PSTACK (JLON, IPSTPT_ZQL+JLEV-(1)) = PQCS(JLON,JLEV)*(1.0_JPRB-ZICE)
    PSTACK (JLON, IPSTPT_ZQI+JLEV-(1)) = PQCS(JLON,JLEV)*ZICE
  ENDDO
ENDDO
...
CALL ACMICRO ( KIDIA, KFDIA, KLON, KTDIA, KLEV, PNEBS, PSTACK (1, IPSTPT_ZT),   &
& PSTACK (1, IPSTPT_ZQL), PSTACK (1, IPSTPT_ZQI), PTS, PNEIJ, PLSM,             &
& PSTACK (1, IPSTPT_ZAUTOL), PSTACK (1, IPSTPT_ZAUTOI), KSTACK, PSTACK, KPSTSZ, &
& KKSTSZ, IPSTPT, IKSTPT )
...
DO JLEV = KTDIA, KLEV
  DO JLON = KIDIA, KFDIA
    ...
    PSTACK (JLON, IPSTPT_ZFLUXCOR+JLEV-(1)) = ZIGEL*&
    &  PSTACK (JLON, IPSTPT_ZAUTOL+JLEV-(1))
    PSTACK (JLON, IPSTPT_ZAUTOL+JLEV-(1)) =   &
    &  PSTACK (JLON, IPSTPT_ZAUTOL+JLEV-(1)) -&
    &  PSTACK (JLON, IPSTPT_ZFLUXCOR+JLEV-(1))
    PSTACK (JLON, IPSTPT_ZAUTOI+JLEV-(1)) =   &
    &  PSTACK (JLON, IPSTPT_ZAUTOI+JLEV-(1)) +&
    &  PSTACK (JLON, IPSTPT_ZFLUXCOR+JLEV-(1))
    PSTACK (JLON, IPSTPT_ZQL+JLEV-(1)) = PSTACK (JLON, IPSTPT_ZQL+JLEV-(1)) - &
    &  PSTACK (JLON, IPSTPT_ZFLUXCOR+JLEV-(1))*YRPHY2%TSPHY
    PSTACK (JLON, IPSTPT_ZQI+JLEV-(1)) = PSTACK (JLON, IPSTPT_ZQI+JLEV-(1)) + &
    &  PSTACK (JLON, IPSTPT_ZFLUXCOR+JLEV-(1))*YRPHY2%TSPHY
    PFCSQL(JLON,JLEV) = PFCSQL(JLON,JLEV-1) + ( &
    & PSTACK (JLON, IPSTPT_ZQL+JLEV-(1)) - PQL(JLON,JLEV) ) * ZDPSGDT
    PFCSQN(JLON,JLEV) = PFCSQN(JLON,JLEV-1) + ( &
    & PSTACK (JLON, IPSTPT_ZQI+JLEV-(1)) - PQI(JLON,JLEV) ) * ZDPSGDT
  ENDDO
ENDDO
...
CALL ADVPRCS ( KIDIA, KFDIA, KLON, KTDIA, KLEV, PSTACK (1, IPSTPT_ZT),            &
& PSTACK (1, IPSTPT_ZQ), PSTACK (1, IPSTPT_ZQL), PSTACK (1, IPSTPT_ZQI),          &
& PSTACK (1, IPSTPT_ZAUTOL), PSTACK (1, IPSTPT_ZAUTOI), PQR, PQS, PNEBS, PCP, PR, &
& PAPHI, PAPRSF, PDELP, PFPLSL , PFPLSN, PFPEVPL, PFPEVPN, PFPFPL, PFPFPN,        &
& PSEDIQL, PSEDIQN, KSTACK, PSTACK, KPSTSZ, KKSTSZ, IPSTPT, IKSTPT )
