...
ASSOCIATE(GUDDD=>YRPHY0%GUDDD, GFRIC=>YRPHY0%GFRIC, GCVFRR=>YRPHY0%GCVFRR, &
 & GMUKEDD=>YRPHY0%GMUKEDD, VVX=>YRPHY0%VVX, VVN=>YRPHY0%VVN, &
 & TENTRX=>YRPHY0%TENTRX, GSDAMIN=>YRPHY0%GSDAMIN, FQLIC=>YRPHY0%FQLIC, &
 & TEQC=>YRPHY0%TEQC, FEVAPC=>YRPHY0%FEVAPC, FENTRT=>YRPHY0%FENTRT, &
 & GSDAMAX=>YRPHY0%GSDAMAX, GCVADET=>YRPHY0%GCVADET, GCVADS=>YRPHY0%GCVADS, &
 & GALMIN=>YRPHY0%GALMIN, FNEBC=>YRPHY0%FNEBC, GEXPKD=>YRPHY0%GEXPKD, &
 & GALMAX=>YRPHY0%GALMAX, SCO=>YRPHY0%SCO, TCW=>YRPHY0%TCW, &
 & GCVCINC=>YRPHY0%GCVCINC, GALTAU=>YRPHY0%GALTAU, GCVWEXC=>YRPHY0%GCVWEXC, &
 & TCA=>YRPHY0%TCA, LGDDD=>YRPHY0%LGDDD, ADOE=>YRPHY0%ADOE, &
 & GREMIN=>YRPHY0%GREMIN, GCVTRMIN=>YRPHY0%GCVTRMIN, &
 & GREMAX=>YRPHY0%GREMAX, GAMAP1=>YRPHY0%GAMAP1, &
 & GCVENTR_MIN=>YRPHY0%GCVENTR_MIN, LCVNAUV=>YRPHY0%LCVNAUV, &
 & LGVVEX=>YRPHY0%LGVVEX, NCVSIG=>YRPHY0%NCVSIG, GSDMIN=>YRPHY0%GSDMIN, &
 & RDTFAC=>YRPHY0%RDTFAC, GCVRE=>YRPHY0%GCVRE, GTDCL=>YRPHY0%GTDCL, &
 & GCVGQX=>YRPHY0%GCVGQX,GINSTX=>YRPHY0%GINSTX,ALFX=>YRPHY0%ALFX, &
 & LCVEOD=>YRPHY0%LCVEOD, LCVUVM=>YRPHY0%LCVUVM, &
 & NCVCLOS=>YRPHY0%NCVCLOS, ECMNP=>YRPHY0%ECMNP, GKEDDRE=>YRPHY0%GKEDDRE, &
 & LCVCONTAU=>YRPHY0%LCVCONTAU, GDEPTH=>YRPHY0%GDEPTH, GCVTEXC=>YRPHY0%GCVTEXC, &
 & RKDN=>YRPHY0%RKDN, GNHDSMOW=>YRPHY0%GNHDSMOW, GNHDSMOS=>YRPHY0%GNHDSMOS, &
 & GSINHR=>YRPHY0%GSINHR, GCVENTRN=>YRPHY0%GCVENTRN, GCHI0=>YRPHY0%GCHI0, &
 & NCVENTR=>YRPHY0%NCVENTR, GCVEZ=>YRPHY0%GCVEZ, GCVENTRX=>YRPHY0%GCVENTRX, &
 & TUDGP=>YRPHY0%TUDGP, GSINHD=>YRPHY0%GSINHD, GSIGA=>YRPHY0%GSIGA, &
 & GNHACC=>YRPHY0%GNHACC, GRBCC=>YRPHY0%GRBCC, LCVIMPT=>YRPHY0%LCVIMPT, &
 & GSDMAX=>YRPHY0%GSDMAX, TENTR=>YRPHY0%TENTR, GNHDEV=>YRPHY0%GNHDEV, &
 & LSBUO=>YRPHY0%LSBUO, GNHDTAU=>YRPHY0%GNHDTAU, TVFC=>YRPHY0%TVFC, &
 & LCVNHD=>YRPHY0%LCVNHD, GFSURF=>YRPHY0%GFSURF, GCLOEB=>YRPHY0%GCLOEB, &
 & TSPHY=>YRPHY2%TSPHY, &
 & NDLON=>YRDIM%NDLON, &
 & NSMAX=>YRDIM%NSMAX, &
 & NPHYREP=>YRPHY%NPHYREP, NBITER=>YRPHY%NBITER, &
 & EDELY=>YREGEO%EDELY, EDELX=>YREGEO%EDELX, &
 & REFLCAPE => YRPHY0%REFLCAPE, LGPRONI1 => YRPHY0%LGPRONI1, NPRONI => YRPHY0%NPRONI, &
 & GPRONI => YRPHY0%GPRONI, GCVUV => YRPHY0%GCVUV, &
 & RSTATI=>YRRIP%RSTATI, &
 & ALFX_LR=>YRPHY0%ALFX_LR, ALFX_HR=>YRPHY0%ALFX_HR, GCVRESN=>YRPHY0%GCVRESN, &
 & GCVRESX=>YRPHY0%GCVRESX, RDELXN=>YRGEM%RDELXN,  NCAPEMOD=>YRPHY0%NCAPEMOD, &
 & WCAPEMOD=>YRPHY0%WCAPEMOD, RENTRMOD=>YRPHY0%RENTRMOD, &
 & RNEBCX=>YRPHY0%RNEBCX)
...
DO JLEV=KTDIA,KLEV
  DO JLON=KIDIA,KFDIA
    ...
    ZT(JLON,JLEV)=MIN(PT(JLON,JLEV),ZPREVIOUST(JLON)+GINSTX)
...
IF(REFLCAPE > 0._JPRB) THEN
  ...
  DO JLON=KIDIA,KFDIA
    ...
    ZFDXTAUX(JLON)=TEQC*1.E3_JPRB*(2.234E-4_JPRB*ZDLONL**2 &
...
ZKDX=RKDN*ZENTRX/ZENTR
ZDEPTH(:)=0._JPRB
ZASCENT_BASE(:)=100000._JPRB
ZDIAGFLO(:,:)=0._JPRB
ZDIAG_ECART_TV(:,:)=0._JPRB
ZDIAG_ECART_T(:,:)=0._JPRB
ZDIAGFRICTION(:,:)=0._JPRB
ZDIAGTRANSP(:,:)=0._JPRB
ZDIAG_EPS_ORG(:,:)=0._JPRB
ZDIAG_EPS_TUR(:,:)=0._JPRB
ZDIAG_WMAX(:)=0._JPRB
ZDIAG_TUDAL=0._JPRB
ZDIAG_KNLAB(:,:)=0._JPRB
ZBCC(:,:)=0._JPRB
ZBCC_C_HALF(:,:)=0._JPRB
ZBCC_C_FULL(:,:)=0._JPRB
ZBCC_CD_FULL(:,:)=0._JPRB

PFHMLTS(:,:)=0._JPRB
PFHEVPP(:,:)=0._JPRB
ZDIFCS_DD_DIAG(:,:)=0._JPRB
ZDIFCS_UD_NOCORR_DIAG(:,:)=0._JPRB
ZDIFCS_DD_NOCORR_DIAG(:,:)=0._JPRB
ZBUO_W(:,:)=0._JPRB
ZBUO_W_POS(:,:)=0._JPRB
...
!DEC$ VECTOR ALWAYS
  DO JLON=KIDIA,KFDIA
    ...
    ZCVGQ(JLON)=ZCVGQ(JLON)+MIN(GCVGQX,PCVGQ(JLON,JLEV)) &
...
DO JLON=KIDIA,KFDIA
  ...
  ELSEIF(LLTEXC) THEN
    ...
    PTU(JLON,KLEV)=ZT(JLON,KLEV)+MAX(0.1_JPRB,MIN(2.0_JPRB,GCVTEXC*PTKE(JLON,KLEV)/RCPD))
  ...
  ELSE
    ...
    PTU(JLON,KLEV)=ZT(JLON,KLEV)+MAX(0._JPRB,GFSURF*(PTS(JLON)-ZT(JLON,KLEV)))
    ! THE PROFILE STARTS AT LOWEST MODEL LEVEL FROM THE GRID-SCALE WATER VAPOUR.
    PQU(JLON,KLEV)=PQ(JLON,KLEV)
  ENDIF
  ...
  ZQW=FOQS(ZESP)
  ZTD=ICDN(JLON)*(GCVADET*ZTD+(1.0_JPRB-GCVADET)*PTU(JLON,KLEV))+(1-ICDN(JLON))*PTU(JLON,KLEV)
  ...
  IF(NCVENTR == 1) THEN
    ZKD(JLON,KLEV)=ZKDX
  ELSE
    ZKD(JLON,KLEV)=ADOE*ZENTRX*PR(JLON,KLEV)*ZT(JLON,KLEV)/PAPRSF(JLON,KLEV)
  ENDIF
...
!   Isolate log (may not vectorize)
    ...
    DO JLON=KIDIA,KFDIA
      ...
      IVVN=NINT(MAX(0.0_JPRB,SIGN(1.0_JPRB,ZVVERDM(JLON,JLEV)*ZBINBUO(JLON,JLEV)-VVN)))
      IVVX=NINT(MAX(0.0_JPRB,SIGN(1.0_JPRB,ZVVERDM(JLON,JLEV)*ZBINBUO(JLON,JLEV)-VVX)))
      ...
      ZEPS_TURL=ZMOD(JLON,JLEV)*(ZENTR+(ZENTRX-ZENTR)*SIN(RPI/2.0_JPRB*MAX(0.0_JPRB,MIN(1.0_JPRB&
         &,((VVX-ZVVERDM(JLON,JLEV))/(VVX-VVN)))))**2)
      ...
      ZKD(JLON,JLEV)=ZEPS_TUR(JLON,JLEV)*RKDN/ZENTR
  ...
  ELSEIF(NCVENTR == 3) THEN
    DO JLON=KIDIA,KFDIA
      ...
      ZKD(JLON,JLEV)=ADOE*ZEPSILON_U(JLON,JLEV)/RG*PR(JLON,JLEV)*ZT(JLON,JLEV)/PAPRSF(JLON,JLEV)
  ...
  ELSEIF(NCVENTR == 16) THEN
    DO JLON=KIDIA,KFDIA
      ...
      ZEPSILON_U(JLON,JLEV)=(ZEPS_ORG(JLON,JLEV)*PAPRSF(JLON,JLEV) &
        &/(PR(JLON,JLEV)*ZT(JLON,JLEV))+ZEPS_TUR(JLON,JLEV))*RG
      ZDELTA_U(JLON,JLEV)=((ZDEL_ORG(JLON,JLEV)*PAPRSF(JLON,JLEV) &
        &/(PR(JLON,JLEV)*ZT(JLON,JLEV))+ZEPS_TUR(JLON,JLEV))*RG)*GCHI0
    ENDDO
...
!   Isolate Log (may not vectorize)
    ...
    DO JLON=KIDIA,KFDIA
      IVVER=NINT(MAX(0.0_JPRB,-SIGN(1.0_JPRB,ZVVERDM(JLON,JLEV)+0.0_JPRB)))
      ZDLOGM=(ZEPS_ORG(JLON,JLEV)-ZDEL_ORG(JLON,JLEV))/GCVRE
...
! Isolate Log (may not vectorize)
  ...
  DO JLON=KIDIA,KFDIA
    ...
    ZESP=ZEW_TLE(JLON)/PAPRSF(JLON,JLEV)
    ZQW=FOQS(ZESP)
    ZTD=ICDN(JLON)*(GCVADET*ZTD+(1.0_JPRB-GCVADET)*PTU(JLON,JLEV))+(1-ICDN(JLON))*PTU(JLON,JLEV)
...
IF(LCVNHD) THEN
  ...
  DO JLEV=KTDIA,KLEV
    DO JLON=KIDIA,KFDIA
      ...
      ZT_DOWN(JLON,JLEV)=ZT(JLON,JLEV)-GNHDEV*ZEVAPO(JLON,JLEV) &
  ...
  ZTWU_TOT_RAW(:,:)=0._JPRB
  ZTWD_TOT_RAW(:,:)=0._JPRB
  ZTWU_TOT(:,:)=0._JPRB
  ZTWD_TOT(:,:)=0._JPRB
  ZTWU_STAU(:,:)=0._JPRB
  ZTWD_STAU(:,:)=0._JPRB
...
IF (ITOP < KLEV+1) THEN
  ...
  DO JLEV=KLEV-1,ITOP,-1
    DO JLON=KIDIA,KFDIA
      ...
      ZFORM(JLON,JLEV)=MAX(0.0_JPRB,ZFORM(JLON,JLEV)-PALF(JLON)/FEVAPC&
        & *ZFORD(JLON,JLEV))
...
!DEC$ IVDEP
    DO JLON=KIDIA,KFDIA
      ...
      PENTR_D(JLON,JLEV)=-ZWD(JLON,JLEV)*PDDAL(JLON,JLEV) &
         ...
         &/(PR(JLON,JLEV)*ZT(JLON,JLEV))+ZENTRX)*RG*IQLIC(JLON)*GCVFRR
      PDETR_D(JLON,JLEV)=-ZWD(JLON,JLEV)*PDDAL(JLON,JLEV) &
         &*(ZDEL_ORGD(JLON,JLEV)*PAPRSF(JLON,JLEV)/(PR(JLON,JLEV)*ZT(JLON,JLEV))+ZENTRX)*RG*GCVFRR
    ENDDO
  ENDDO
  ...
  DO JLEV=KLEV-1,ITOP,-1
    DO JLON=KIDIA,KFDIA
       ...
       DO JTRA=1,KTRA
         PSTRCTRA(JLON,JLEV,JTRA)=(1._JPRB-ZA)*PSTRCTRA(JLON,JLEV,JTRA)+ZA2*(PSTRCTRA(JLON,JLEV-1,JTRA)+PSTRCTRA(JLON,JLEV+1,JTRA))
       ENDDO
    ENDDO
  ENDDO
