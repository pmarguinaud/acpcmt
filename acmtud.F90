...
REAL(KIND=JPRB)   , INTENT(INOUT) :: PALF
...
REAL(KIND=JPRB) :: ZKEDD
REAL(KIND=JPRB) :: ZTALF_SCE
REAL(KIND=JPRB) :: ZTALF_SIN
REAL(KIND=JPRB) :: ZTBW
REAL(KIND=JPRB) :: ZQBW
...
temp (REAL(KIND=JPRB), ZWU, (KLON,KLEV+1))
temp (REAL(KIND=JPRB), ZWD, (KLON,KLEV+1))
temp (REAL(KIND=JPRB), ZWD_PLUS_BUO, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZDQCA, (KLON,0:KLEV))
temp (REAL(KIND=JPRB), ZFORM, (KLON,0:KLEV))
temp (REAL(KIND=JPRB), ZVM, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZUM, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTVN, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTVE, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZSDN, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZRMIX, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZPOID, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZLDN, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZIFRAC, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZDSE, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZBET, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZA14, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZA13, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZA15, (KLON,KLEV,KTRA))
temp (REAL(KIND=JPRB), ZTRAM, (KLON,KLEV,KTRA))
temp (REAL(KIND=JPRB), ZTRAMD, (KLON,KLEV,KTRA))
temp (REAL(KIND=JPRB), ZSTRCTRAD, (KLON,0:KLEV,KTRA))
temp (INTEGER(KIND=JPIM), INBAS, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZDWNDP, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZVVER, (KLON,0:KLEV))
temp (REAL(KIND=JPRB), ZDEL_ORG, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZEPS_ORG, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZEPS_TUR, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZSIGMA, (KLON,0:KLEV))
temp (REAL(KIND=JPRB), ZKD, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZVVERDM, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZBUO_W, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZBUO_W_POS, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZMOD, (KLON,KLEV))
...
REAL(KIND=JPRB) :: ZCAPEL
REAL(KIND=JPRB) :: ZRBHS
REAL(KIND=JPRB) :: ZRBBS

REAL(KIND=JPRB) :: ZCINL
REAL(KIND=JPRB) :: ZCIN
...
REAL(KIND=JPRB) :: ZS11
REAL(KIND=JPRB) :: ZS12
REAL(KIND=JPRB) :: ZFMDQ
REAL(KIND=JPRB) :: ZFMDS
REAL(KIND=JPRB) :: ZDLON,ZDLONL,ZFORMV
REAL(KIND=JPRB) :: ZTAU_DIAG
REAL(KIND=JPRB) :: ZFDXTAUX
REAL(KIND=JPRB) :: ZTD
REAL(KIND=JPRB) :: ZDIVERG,ZBIN,ZTKLEV
REAL(KIND=JPRB) :: ZQBASE
REAL(KIND=JPRB) :: ZTBASE
REAL(KIND=JPRB) :: ZDEPTH
REAL(KIND=JPRB) :: ZASCENT_BASE

temp (REAL(KIND=JPRB), ZBUOY, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZQDN, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZEMD_DIAG, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZENTR_T_LOC, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZFLUXDIAG, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZDELTA_U, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZEPSILON_U, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZVACC, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZDIAGFLO, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZDIAG_ECART_TV, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZDIAG_ECART_T, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZDIAGFRICTION, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZDIAGTRANSP, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZDIAG_EPS_ORG, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZDIAG_EPS_TUR, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZDIAG_TUDAL, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZDIAG_KNLAB, (KLON,KLEV))
temp (INTEGER(KIND=JPIM), INIVBAC, (KLON,KLEV))
temp (INTEGER(KIND=JPIM), INCDN, (KLON,KLEV))
REAL(KIND=JPRB) :: ZDIAG_WMAX
INTEGER(KIND=JPIM) :: ISDELTAP
INTEGER(KIND=JPIM) :: IBAC
INTEGER(KIND=JPIM) :: ICDN
INTEGER(KIND=JPIM) :: IVVER,IDOMDP,IVVN,IVVX,INUA,ICHIS,IENTR
INTEGER(KIND=JPIM) :: IQLIC
INTEGER(KIND=JPIM) :: IKUO6
INTEGER(KIND=JPIM) :: IE0,ICIN

INTEGER(KIND=JPIM) :: ILFC
INTEGER(KIND=JPIM) :: ILEVTEN
INTEGER(KIND=JPIM) :: IKUO5
...
temp (INTEGER(KIND=JPIM), INLFC, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTU, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZQU, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZEPS_ORGD, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZDEL_ORGD, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZBETD, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTN2D, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZSN2D, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZSDND, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZQN2D, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZQDND, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZFORD, (KLON,0:KLEV))
temp (REAL(KIND=JPRB), ZFORU, (KLON,0:KLEV))
temp (REAL(KIND=JPRB), ZVMD, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZUMD, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZBCC_C_HALF, (KLON,0:KLEV))
temp (REAL(KIND=JPRB), ZBCC_CD_FULL, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZBCC_C_FULL, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZBCC, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZRHO, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZT_DOWN, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZDIFF_Z_UD, (KLON,0:KLEV))
temp (REAL(KIND=JPRB), ZTWU_ADV, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTWU_BUO, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTWU_DYP, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTWU_NHD, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTWU_FRI, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTWU_TOT_RAW, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTWU_TOT, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTWU_STAU, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTWD_ADV, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTWD_BUO, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTWD_DYP, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTWD_NHD, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTWD_FRI, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTWD_TOT_RAW, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTWD_TOT, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTWD_STAU, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZMEAN_U, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZMEAN_D, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZINTEG, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTARGET_WD, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZDIFCS_DD_DIAG, (KLON,0:KLEV))
...
temp (REAL(KIND=JPRB), ZDIFCS_UD_NOCORR_DIAG, (KLON,0:KLEV))
temp (REAL(KIND=JPRB), ZDIFCS_DD_NOCORR_DIAG, (KLON,0:KLEV))
 ! Reduce Entrainment factor, total.
temp (REAL(KIND=JPRB), ZRE, (KLON,KLEV))
 ! Reduce Entrainment factor, due to saturation deficit.
temp (REAL(KIND=JPRB), ZRE_SATDEF, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZRE_HEIGHT, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZRE_KEDD, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZBINID2, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZBINID, (KLON,KLEV))

REAL(KIND=JPRB) :: ZRH
REAL(KIND=JPRB) :: ZINTE
REAL(KIND=JPRB) :: ZDENO
REAL(KIND=JPRB) :: ZSATDEF
REAL(KIND=JPRB) :: ZRT,ZIWGREAT
REAL(KIND=JPRB) :: ZBINTKE,ZBINDEPTH,ZHEIGHT
temp (REAL(KIND=JPRB), ZBINBUO, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZDIFCSD, (KLON,0:KLEV))
temp (REAL(KIND=JPRB), ZDIFCQD, (KLON,0:KLEV))
temp (REAL(KIND=JPRB), ZSTRCVD, (KLON,0:KLEV))
temp (REAL(KIND=JPRB), ZSTRCUD, (KLON,0:KLEV))
REAL(KIND=JPRB) :: ZNND
temp (REAL(KIND=JPRB), ZI1, (KLON,KLEV))
 ! geopotential at full levels within updraft, under the equipressure hypothesis.
temp (REAL(KIND=JPRB), ZPHIF_U_EQUIP, (KLON,KLEV+1))
 ! geopotential at full levels within updraft.
temp (REAL(KIND=JPRB), ZPHIF_U, (KLON,KLEV+1))
temp (REAL(KIND=JPRB), ZEVAPO, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZTMPVAR, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZLISS, (KLON,KLEV))
temp (REAL(KIND=JPRB), ZRHOH, (KLON,0:KLEV))
temp (REAL(KIND=JPRB), ZT, (KLON,KLEV))

REAL(KIND=JPRB) :: ZALFAVEC
REAL(KIND=JPRB) :: ZLOGRATIO

REAL(KIND=JPRB) :: ZEW_TLN
REAL(KIND=JPRB) :: ZEW_TLE
REAL(KIND=JPRB) :: ZTLN_ARR
REAL(KIND=JPRB) :: ZTLE_ARR

REAL(KIND=JPRB) :: ZDELTA_ARR
REAL(KIND=JPRB) :: ZLOGSIGMA

REAL(KIND=JPRB) :: ZPREVIOUST
REAL(KIND=JPRB) :: ZALF_CVGQ
REAL(KIND=JPRB) :: ZIBCCS
REAL(KIND=JPRB) :: ZCVGQ

REAL(KIND=JPRB) :: ZALFX
...
INTEGER(KIND=JPIM) :: IEL
INTEGER(KIND=JPIM) :: IFCL
INTEGER(KIND=JPIM) :: ILCL

temp (INTEGER(KIND=JPIM), INLABD, (KLON,KLEV))
...
alloc (ZINTEG)
alloc (ZTARGET_WD)
alloc (ZDIFCS_DD_DIAG)
alloc (ZDIFCS_UD_NOCORR_DIAG)
alloc (ZDIFCS_DD_NOCORR_DIAG)
alloc (ZRE)
alloc (ZRE_SATDEF)
alloc (ZRE_HEIGHT)
alloc (ZRE_KEDD)
...
DO JLEV=KTDIA,KLEV
    ...
    ZT(JLON,JLEV)=MIN(PT(JLON,JLEV),ZPREVIOUST+YRPHY0%GINSTX)
    ZPREVIOUST=ZT(JLON,JLEV)
...
DO JLEV=KTDIA,KLEV
    ...
    ZKEDD=ZKEDD+0.5_JPRB*ZWD(JLON,JLEV)*ZWD(JLON,JLEV)*PDELP(JLON,JLEV)/RG
...
IF(YRPHY0%REFLCAPE > 0._JPRB) THEN
  ...
  ENDIF
    ...
    ZDLONL=MIN(ZDLON*PGM,950._JPRB)
    ZFDXTAUX=YRPHY0%TEQC*1.E3_JPRB*(2.234E-4_JPRB*ZDLONL**2 &
...
!DEC$ VECTOR ALWAYS
    ...
    ZDIAG_WMAX=MAX(ZDIAG_WMAX,ZWU(JLON,JLEV))
    ...
    ZDEPTH=ZDEPTH+(PAPHI(JLON,JLEV-1)-PAPHI(JLON,JLEV))/RG*ZBINID(JLON,JLEV)
    ...
    ZASCENT_BASE=ZASCENT_BASE*(1._JPRB-ZIWGREAT)+PAPHI(JLON,JLEV)/RG*ZIWGREAT
...
ENDDO
  ...
  ZSATDEF=ZSATDEF/MAX(100._JPRB,ZDENO)
  ZRH=ZRH/MAX(100._JPRB,ZDENO)
...
! THETAE
  ...
  ZTEN=ZT(JLON,KTDIA)*(100000._JPRB/PAPRSF(JLON,KTDIA))&
...
DO JLEV=KTDIA,KLEV
    ...
    ZALFAVEC=MAX(ZEPS0,PAPRSF(JLON,JLEV)*PQ(JLON,JLEV)&
      &/((1.0_JPRB-PQ(JLON,JLEV))*RD/RV+PQ(JLON,JLEV)))
    ZDELTA_ARR=100000._JPRB/PAPRSF(JLON,JLEV)
    ZTLE_ARR=PQ(JLON,JLEV)/(1.0_JPRB-PQ(JLON,JLEV))*1000._JPRB
    ZLOGSIGMA=NINT(MAX(0.0_JPRB,-SIGN(1.0_JPRB,ZEPS0-ZALFAVEC)))
  
  
    IE0=ZLOGSIGMA
    ZR=ZTLE_ARR
    ZTCOND=IE0*2840._JPRB/(3.5_JPRB*LOG(ZT(JLON,JLEV))-LOG(ZALFAVEC/100._JPRB)-4.805_JPRB) &
     & +55._JPRB+(1-IE0)*ZT(JLON,JLEV)
    ZTEL=ZT(JLON,JLEV)*ZDELTA_ARR &
     &**(0.2854_JPRB*(1.0_JPRB-0.28E-3_JPRB*ZR)) &
     &*EXP((3.376_JPRB/ZTCOND-0.00254_JPRB)*ZR*(1.0_JPRB+0.81E-3_JPRB*ZR))
...
!DEC$ VECTOR ALWAYS
    ...
    ZTEN=MIN(ZTEN,ZTEL)
    ILEVTEN=MAX(JLEV*NINT(MAX(0.0_JPRB,SIGN(1.0_JPRB,ZTEN&
      &-ZTEL))),ILEVTEN)
...
IF(LLNHACC) THEN
  ...
  DO JLEV=ILEV,KLEV
      ...
      ZBIN=ZBIN*MAX(0._JPRB,SIGN(1._JPRB,ZTKLEV-ZTBASE))
      ZTBASE=ZBIN*ZTKLEV+(1._JPRB-ZBIN)*ZTBASE
      ZQBASE=ZBIN*PQ(JLON,JLEV)+(1._JPRB-ZBIN)*ZQBASE
...
IF(YRPHY0%GCLOEB > 0._JPRB) THEN
  DO JLEV=KTDIA,KLEV
      ...
      ZRE(JLON,JLEV)=YRPHY0%GREMIN+(YRPHY0%GREMAX-YRPHY0%GREMIN) &
        & *MAX(0._JPRB,MIN(1._JPRB,(ZSATDEF-YRPHY0%GSDMIN)/(YRPHY0%GSDMAX-YRPHY0%GSDMIN)))
...
! KNND     : PHYSICAL SOLUTION FLAG
  ...
  IF(LLNHACC) THEN
    ...
    PTU(JLON,KLEV)=ZTBASE
    PQU(JLON,KLEV)=ZQBASE
  ELSEIF(LLTEXC) THEN
  ...
  ELSE
    ...
    PTU(JLON,KLEV)=ZT(JLON,KLEV)+MAX(0._JPRB,YRPHY0%GFSURF*(PTS-ZT(JLON,KLEV)))
    ! THE PROFILE STARTS AT LOWEST MODEL LEVEL FROM THE GRID-SCALE WATER VAPOUR.
    PQU(JLON,KLEV)=PQ(JLON,KLEV)
  ENDIF
  ...
  ZTVN(JLON,KLEV)=PTU(JLON,KLEV)*(1.0_JPRB+RETV*PQU(JLON,KLEV)-ZLN)
  ...
  ICDN=NINT(MAX(0.0_JPRB,SIGN(1.0_JPRB,PQU(JLON,KLEV)-PQSAT(JLON,KLEV))))
  ...
  ZQW=FOQS(ZESP)
  ZTD=ICDN*(YRPHY0%GCVADET*ZTD+(1.0_JPRB-YRPHY0%GCVADET)*PTU(JLON,KLEV))+(1-ICDN)*PTU(JLON,KLEV)
  ZQW=ICDN*(YRPHY0%GCVADET*ZQW+(1.0_JPRB-YRPHY0%GCVADET)*PQU(JLON,KLEV))+(1-ICDN)*PQU(JLON,KLEV)
  ZQDN(JLON,KLEV)=ZQW
...
!DEC$ VECTOR ALWAYS
    ...
    ZTBW=PTU(JLON,JLEV+1)+ZRMIX(JLON,JLEV)*(PTW(JLON,JLEV+1)-PTU(JLON,JLEV+1))
    ZDELTA_ARR=MAX(0.0_JPRB,SIGN(1.0_JPRB,RTT-ZTBW))

  

! Isolate what may not vectorize
  
    ZEW_TLE= FOEW (ZTBW,ZDELTA_ARR)
  

  

    ZQBW=PQU(JLON,JLEV+1)+ZRMIX(JLON,JLEV)*(PQW(JLON,JLEV+1)-PQU(JLON,JLEV+1))
    ZTB=PTU(JLON,JLEV+1)+ZRMIX(JLON,JLEV)*(ZT(JLON,JLEV+1)-PTU(JLON,JLEV+1))
    ZQB=PQU(JLON,JLEV+1)+ZRMIX(JLON,JLEV)*(PQ(JLON,JLEV+1)-PQU(JLON,JLEV+1))
    ICDN=MAX(0,-ISIGN(1,-INCDN(JLON,JLEV+1)))
    ZLB=ZLN+ZRMIX(JLON,JLEV)*(PQLIS(JLON,JLEV+1)*ICDN-ZLN)
    ZTBH=ZTNH+ZRMIX(JLON,JLEV)*(ZT(JLON,JLEV+1)-ZTNH)
    ZQBH=ZQNH+ZRMIX(JLON,JLEV)*(PQ(JLON,JLEV+1)-ZQNH)
    ...
    ZLHTBW=FOLH(ZTBW,ZDELTA_ARR)
    ZCPBW=RCPD+ZCPVMD*ZQBW

    ! COMPUTATION OF NEW EQUILIBRIUM OF MIXED AIR
    ZESP=ZEW_TLE/PAPRSF(JLON,JLEV+1)
    ZQW= FOQS (ZESP)
    ZDQW=FODQS (ZQW,ZESP, FODLEW (ZTBW,ZDELTA_ARR))
    ZINCRQ=(ZDQW*(ZCPBW*(ZTB-ZTBW)+ZLHTBW&
      &*(ZQB-ZQBW))&
      &+(ZQW-ZQBW)*ZCPBW)/(ZCPBW+ZLHTBW*ZDQW)
    ZQBW=ZQBW+ZINCRQ
    ZQBW=MIN(ZQBW,ZQB+ZLB)
    ...
    ZRBBS=(PLNPR(JLON,JLEV+1)-PALPH(JLON,JLEV+1))*(RD*(1.0_JPRB &
      &-ZLB)+ZRVMD*ZQB)
    ZRBHS=PALPH(JLON,JLEV)*(RD*(1.0_JPRB-ZLB)+ZRVMD*ZQB)
    ZRBB=(PLNPR(JLON,JLEV+1)-PALPH(JLON,JLEV+1))*(RD*(1.0_JPRB &
      &-ZLB)+ZRVMD*ZQBH)
    ZRBH=PALPH(JLON,JLEV)*(RD*(1.0_JPRB-ZLB)+ZRVMD*ZQBH)
    ZRVH=PALPH(JLON,JLEV)*RV

    ! TO COMPUTE SATURATED ADIABATIC PROFILE, GCVADS ALLOWS TO GO
    ! CONTINUOUSLY FROM EQUIPRESSURE TO EQUIGEOPOTENTIAL APPROACH.

    ZRBB=(1.0_JPRB-YRPHY0%GCVADS)*ZRBB+YRPHY0%GCVADS*(PAPHIF(JLON,JLEV)&
      & -PAPHIF(JLON,JLEV+1))/ZTBH
    ZRBH=(1.0_JPRB-YRPHY0%GCVADS)*ZRBH
    ZRBBS=(1.0_JPRB-YRPHY0%GCVADS)*ZRBBS+YRPHY0%GCVADS*(PAPHIF(JLON,JLEV)&
      & -PAPHIF(JLON,JLEV+1))/ZTB
    ZRBHS=(1.0_JPRB-YRPHY0%GCVADS)*ZRBHS
    ZRVH=(1.0_JPRB-YRPHY0%GCVADS)*ZRVH

    ! SNOW OPTION DEPENDENT CALCULATIONS.
    ! DIAGNOSTIC ICE FRACTION
    ! ZDELTA=FONICE(ZTB(JLON))
    ZDELTA=MAX(0.0_JPRB,SIGN(1.0_JPRB,RTT-ZTBH))
    ...
    ZLH=FOLH(ZTBH,ZDELTA)+ZRVH*ZTBH
    ZCP=RCPD+ZCPVMD*ZQBH+(ZCPWMD+ZDELTA*(ZCPSMD-&
      &ZCPWMD))*ZLB+ZRBH
    ZDCP=ZRVH+ZCPVMW+ZDELTA*(ZCPVMS-ZCPVMW)
    ...
    ZLH=ZLH+ZDCP*ZDELT
    ZDELQ=-((ZRBB+ZRBH)*ZTBH+ZCP*ZDELT)/ZLH
    ZTNH=ZTBH+ZDELT
    ZQNH=ZQBH+ZDELQ
    ZCP=ZCP+ZDCP*ZDELQ
...
!   Isolate what may not vectorize
      ...
      ! DIAGNOSTIC ICE FRACTION
      ZDELTA_ARR=MAX(0.0_JPRB,SIGN(1.0_JPRB,RTT-ZTNH))

      ! SATURATION CALCULATIONS USING DEFINED FUNCTIONS.

      ZEW_TLE= FOEW (ZTNH,ZDELTA_ARR)

    
    
      ZESP=ZEW_TLE/PAPRSF(JLON,JLEV)
      ZQW= FOQS (ZESP)
      ZDQW= FODQS (ZQW,ZESP, FODLEW (ZTNH,ZDELTA_ARR))

      ! INCREMENTATIONS.

      ZDCP=ZRVH+ZCPVMW+ZDELTA_ARR*(ZCPVMS-ZCPVMW)
      ZDELQ=(ZQW-ZQNH)*ZCP/(ZCP+ZLH*ZDQW)
      ZCP=ZCP+ZDCP*ZDELQ
      ZDELT=-ZDELQ*ZLH/ZCP
      ZLH=ZLH+ZDCP*ZDELT
      ZQNH=ZQNH+ZDELQ
...
! Isolate log (may not vectorize)
    ...
    ZLOGRATIO=LOG(PAPRSF(JLON,JLEV+1)/PAPRSF(JLON,JLEV))
  
  

    ! // DRY ADIABAT.
    ZCPH=RCPD*(1.0_JPRB-ZQB)+RCPV*ZQB
    ZTNSEC=(ZCPH-ZRBBS)*ZTB/(ZCPH+ZRBHS)

    ! // TEST IF SATURATED AT TOP OF LAYER, INITIALIZE VARIABLES FOR NEXT LEVEL.
    ICDN=NINT(MAX(0.0_JPRB,SIGN(1.0_JPRB,ZTNH-ZTNSEC)))
    INCDN(JLON,JLEV)=MAX(INCDN(JLON,JLEV+1),JLEV*ICDN)
    ICDN=MAX(0,-ISIGN(1,-INCDN(JLON,JLEV)))
    PTU(JLON,JLEV)=ICDN*ZTNH+(1-ICDN)*ZTNSEC
    PQU(JLON,JLEV)=ICDN*ZQNH+(1-ICDN)*ZQB

    ! Buoyant convective condensation.
    ZBCC_C_HALF(JLON,JLEV)=MIN(0._JPRB,PQU(JLON,JLEV)-ZQB) &
    ...
    ZLIQ=YRPHY0%ECMNP/(ZRBB*ZTBH+(ZRBH+ZRVH&
      &*(PQU(JLON,JLEV)-ZQBH))*PTU(JLON,JLEV))
    ZEXP=EXP(-1.0_JPRB/MAX(ZARLII,ZLIQ))
    ZLN=ICDN*MAX(0.0_JPRB,ZLB*ZEXP &
      &+(PQU(JLON,JLEV)-ZQBH)*ZLIQ*(ZEXP-1.0_JPRB))
    PQLIC(JLON,JLEV)=ZLN
    !ZDQCA(JLON,JLEV)=ZQBW(JLON)-PQU(JLON,JLEV) formule ref
    ZDQCA(JLON,JLEV)=ZQB-PQU(JLON,JLEV) ! approche 3MT.

    ZLDN(JLON,JLEV)=ZLN
    ZSDN(JLON,JLEV)=(RCPD+ZCPVMD*PQU(JLON,JLEV))*PTU(JLON,JLEV)&
      &+ZPHIF_U(JLON,JLEV)
    ZDQCA(JLON,JLEV)=ICDN*ZDQCA(JLON,JLEV)

    ! // TEST IF LFC REACHED AT TOP OF LAYER
    ILFC=NINT(MAX(0.0_JPRB,-SIGN(1.0_JPRB,PR(JLON,JLEV)*ZT(JLON,JLEV) &
      & -(RD*(1.0_JPRB-ZLN)+ZRVMD*ZQNH)*ZTNH)))
    INLFC(JLON,JLEV)=MAX(INLFC(JLON,JLEV+1),JLEV*ILFC)
    ILFC=MAX(0,-ISIGN(1,-INLFC(JLON,JLEV)))

    ZTVN(JLON,JLEV)=PTU(JLON,JLEV)*(1.0_JPRB+RETV*PQU(JLON,JLEV)-ZLN)
    ...
    IF (LLCOND1) THEN
      ZCAPEL=RD*PDELP(JLON,JLEV)/PAPRSF(JLON,JLEV)*(ZTVN(JLON,JLEV)-ZTVE(JLON,JLEV))
      ZCINL=-NINT(MAX(0.0_JPRB,-SIGN(1.0_JPRB,ZCAPEL))) &
       & *ZCAPEL*IBAC*ICDN*(1-ILFC)
    ELSE
      ZCAPEL=RD*PDELP(JLON,JLEV)/PAPRSF(JLON,JLEV)*MAX(0._JPRB,ZTVN(JLON,JLEV)-ZTVE(JLON,JLEV))
    ENDIF
...
! REK.
    ...
    ZS5=ZS5+KNLAB(JLON,JLEV+1)*ZCIN
    ZCIN=ZCINL
    IVVER=NINT(MAX(0.0_JPRB,-SIGN(1.0_JPRB,(ZVVERN+ZVVER(JLON,JLEV))+0.0_JPRB)))
    ICIN=(1-IVVER)*IBAC*ICDN*(1-ILFC)&
      &*NINT(MAX(0.0_JPRB,-SIGN(1.0_JPRB,(ZS5+ZCINL-YRPHY0%GCVCINC)+0.0_JPRB)))
    ...
    ZALFAVEC=1.0_JPRB ! no smoothing.
    ZDWNDP(JLON,JLEV)=ZALFAVEC*ZDWNDP(JLON,JLEV)+(1.0_JPRB-ZALFAVEC)*ZDWNDP(JLON,JLEV+1)
  

  IF(YRPHY0%NCVENTR == 1) THEN
!   Isolate log (may not vectorize)
    
      ZTLE_ARR=ZT(JLON,JLEV)-PLH(JLON,JLEV)*PQLIS(JLON,JLEV)/PCP(JLON,JLEV)
      ZDELTA=MAX(0.0_JPRB,SIGN(1.0_JPRB,RTT-ZTLE_ARR))
      ZEW_TLE=FOEW(ZTLE_ARR,ZDELTA)
      ZTLN_ARR=PTU(JLON,JLEV)-ZLH*ZLN/ZCP
      ZDELTA=MAX(0.0_JPRB,SIGN(1.0_JPRB,RTT-ZTLN_ARR))
      ZEW_TLN=FOEW(ZTLN_ARR,ZDELTA)
      ...
      ZESP=ZEW_TLE/PAPRSF(JLON,JLEV)
      ZQSTLE=FOQS(ZESP)
      ZESP=ZEW_TLN/PAPRSF(JLON,JLEV)
      ZQSTLN=FOQS(ZESP)
      ZSN=PQU(JLON,JLEV)+ZLN-ZQSTLN
      ...
      ZTVLN=ZTLN_ARR*(1.0_JPRB+RETV*(PQU(JLON,JLEV)+ZLN))
      ZTVLE=ZTLE_ARR*(1.0_JPRB+RETV*(PQ(JLON,JLEV)+PQLIS(JLON,JLEV)))
  ...
  ELSEIF(YRPHY0%NCVENTR == 3) THEN
      ...
      ZTVNL=PTU(JLON,JLEV)*(1.0_JPRB+RETV*PQU(JLON,JLEV)-ZLN)
  ...
  ELSEIF(YRPHY0%NCVENTR == 16) THEN
      ...
      ZTLN=PTU(JLON,JLEV)-ZLH*ZLN/ZCP
      ...
      ZCHIS=MIN(1.0_JPRB,MAX(0.0_JPRB,ZCHIS))
      ICHIS=NINT(MAX(0.0_JPRB,-SIGN(1.0_JPRB,-ZCHIS+0.0_JPRB)))
      ZTVLN=ZTLN*(1.0_JPRB+RETV*(PQU(JLON,JLEV)+ZLN))
  ...
  ELSEIF(YRPHY0%NCVENTR == 17) THEN
      ...
      ZEPSILON_U(JLON,JLEV)=ZENTRX*RG
      ZKD(JLON,JLEV)=YRPHY0%ADOE**(1._JPRB+ZDEPTH/7000._JPRB*24._JPRB) &
  ...
  ELSEIF(YRPHY0%NCVENTR == 19) THEN
      ...
      ZRE_KEDD(JLON,JLEV)=1._JPRB/(1._JPRB+YRPHY0%GKEDDRE*ZKEDD)
...
! Isolate Log (may not vectorize)
    ...
    ZTD=ZT(JLON,JLEV)*(1.0_JPRB+(ZLN-RETV*(PQSAT(JLON,JLEV)&
      & -PQ(JLON,JLEV)))/(1.0_JPRB+RETV*PLH(JLON,JLEV)*PQSAT(JLON,JLEV)&
      & /(RV*ZT(JLON,JLEV))))
    ZDELTA=MAX(0.0_JPRB,SIGN(1.0_JPRB,RTT-ZTD))
    ZEW_TLE=FOEW (ZTD,ZDELTA)
  ...
  ENDIF
    ...
    IBAC=MAX(0,-ISIGN(1,-INIVBAC(JLON,JLEV+1)))
    IKUO5=MAX(0,ISIGN(1,&
     & IBAC*INIVBAC(JLON,JLEV+1)+(1-IBAC) &
     & -ILEVTEN))
    ...
    ZESP=ZEW_TLE/PAPRSF(JLON,JLEV)
    ZQW=FOQS(ZESP)
    ZTD=ICDN*(YRPHY0%GCVADET*ZTD+(1.0_JPRB-YRPHY0%GCVADET)*PTU(JLON,JLEV))+(1-ICDN)*PTU(JLON,JLEV)
    ZQW=ICDN*(YRPHY0%GCVADET*ZQW+(1.0_JPRB-YRPHY0%GCVADET)*PQU(JLON,JLEV))+(1-ICDN)*PQU(JLON,JLEV)
    ...
    ZTNH=INUA*ZTNH+(1-INUA)*PTW(JLON,JLEV)
    ZQNH=INUA*ZQNH+(1-INUA)*PQW(JLON,JLEV)

    ! // SATURATION TEST AT TOP LEVEL.
    ICDN=NINT(MAX(0.0_JPRB,SIGN(1.0_JPRB,PQU(JLON,JLEV)-PQSAT(JLON,JLEV))))
    ...
    ZS15=ZS15+KNLAB(JLON,JLEV+1)*ZBINBUO(JLON,JLEV)*ZCAPE
    ...
    ZS16=ZS16+KNLAB(JLON,JLEV+1)*ZBINBUO(JLON,JLEV)&
       &*RD*((1.0_JPRB+RETV*PQ(JLON,JLEV+1))/PCP(JLON,JLEV+1)&
       &*(ZFMDS-ZFMDSN+ZFORMV*PLH(JLON,JLEV+1)&
       &*PDELP(JLON,JLEV+1)*ZBCC_C_FULL(JLON,JLEV+1))&
       &+RETV*ZT(JLON,JLEV+1)*(ZFMDQ-ZFMDQN&
    ...
    ZS11=ZS11+ZDPLAB*ABS(ZVVERDM(JLON,JLEV+1))
    ZCAPE=ZCAPEL
...
ENDDO
  ...
  DO JLEV=KTDIA,KLEV
      ...
      ZT_DOWN(JLON,JLEV)=ZT(JLON,JLEV)-YRPHY0%GNHDEV*ZEVAPO(JLON,JLEV) &
        & *MAX(0._JPRB,SIGN(1._JPRB,ZDEPTH-YRPHY0%GDEPTH))
...
IF (ITOP < KLEV+1) THEN
  ...
  ! LAST LEVEL SUMMATION (TOP).
    ...
    ZS15=ZS15+KNLAB(JLON,KTDIA)*ZBINBUO(JLON,KTDIA)*ZCAPE
    ...
    ZS16=ZS16+KNLAB(JLON,KTDIA)*ZBINBUO(JLON,KTDIA)&
       &*RD*((1.0_JPRB+RETV*PQ(JLON,KTDIA))/PCP(JLON,KTDIA)&
       &*(ZFMDS-ZFMDSN+ZFORMV*PLH(JLON,KTDIA)&
       &*PDELP(JLON,KTDIA)*ZBCC_C_FULL(JLON,KTDIA))&
       &+RETV*ZT(JLON,KTDIA)*(ZFMDQ-ZFMDQN&
    ...
    ZS12=ZS12+ZDPLAB
    ZS11=ZS11+ZDPLAB*ABS(ZVVERDM(JLON,KTDIA))
...
ZIBCCS = 0.0_JPRB
 ...
 ! Integral of BCC multiplied by Sigma.
  DO JLEV=ITOP,KLEV
      ...
      ZIBCCS=ZIBCCS-PUDOM(JLON,JLEV) &
        & *ZBCC_C_FULL(JLON,JLEV)*ZSIGMA(JLON,JLEV) &
        & *ZBINID(JLON,JLEV)*PDELP(JLON,JLEV)/RG
    
    IF (LLCAPECIN) THEN
      
        PCAPE=PCAPE +KNLAB(JLON,JLEV)*RD&
  ...
  ENDDO
    ...
    ZTAU=1.0_JPRB/ZFDXTAUX*ZS12*ZS12/MAX(ZS11,ZEPS0)
    ZTAU_DIAG=ZTAU
    ! Impose tau (time consumption of CAPE) to be larger than the model time step.
    IF(YRPHY0%LCVCONTAU) ZTAU=MAX(YRPHY2%TSPHY,ZTAU)
    ZS15=MAX(0.0_JPRB,ZS15)
    ...
    ! prediction.
    ZALF_CVGQ=MAX(0._JPRB,ZCVGQ)/MAX(ZEPS_BCCS,ZIBCCS) &
      & *MAX(0._JPRB,SIGN(1._JPRB,ZIBCCS-ZEPS_BCCS)) &
      & *MAX(0._JPRB, SIGN(1._JPRB,YRRIP%RSTATI-3600._JPRB))


    ! EXTRA-DIAGNOSTICS
    PALF_CVGQ=ZALF_CVGQ
    PALF_CAPE=ZS15/MAX(ZEPS16,ZS16)&
        & *MAX(0.0_JPRB,SIGN(1.0_JPRB,(ZS16-ZEPS16)))&
        & /MAX(ZTAU,ZEPS0)

    IF(YRPHY0%NCVCLOS == 1) THEN
      ! ALPHA DIAGNOSED FROM d(CAPE)/dt=-CAPE/TAU.
      PALF=MIN(ZALFX,ZS15/MAX(ZEPS16,ZS16)&
        & *MAX(0.0_JPRB,SIGN(1.0_JPRB,(ZS16-ZEPS16)))&
        & /MAX(ZTAU,ZEPS0))

    ELSEIF(YRPHY0%NCVCLOS == 2) THEN
      ! ALPHA AS A FUNCTION OF SATURATION DEFICIT.
      PALF=YRPHY0%GALMIN+(YRPHY0%GALMAX-YRPHY0%GALMIN) &
      & *MAX(0._JPRB,MIN(1._JPRB,(YRPHY0%GSDAMAX-ZSATDEF) &
      ...
      ZBINDEPTH=MAX(0._JPRB,SIGN(1._JPRB,800._JPRB-ZDEPTH))
      PALF=PALF*(1._JPRB-ZBINTKE*ZBINDEPTH)

      ! TAU COMPUTED HERE FOR DIAGNOSTIC PURPOSE ONLY.
      ZTAU=ZS15/MAX(ZEPS16,ZS16)/MAX(0.001_JPRB,PALF)

      ! Impose time consumption of CAPE to be larger than the model time step.
      IF(YRPHY0%LCVCONTAU) PALF=PALF*MAX(0._JPRB,MIN(1._JPRB,ZTAU/YRPHY2%TSPHY))

    ELSEIF(YRPHY0%NCVCLOS == 3) THEN
    ! ALPHA AS A FUNCTION OF DOWNDRAFT ACTIVITY.
      PALF=YRPHY0%GALMIN+(YRPHY0%GALMAX-YRPHY0%GALMIN) &
      & *MAX(0._JPRB,MIN(1._JPRB,ZKEDD/7500._JPRB))
    ...
    ELSEIF(YRPHY0%NCVCLOS == 4) THEN
      ! PALF is prognostic.
      ZTALF_SCE=PALF*YRPHY0%GMUKEDD*ZKEDD
      ZTALF_SIN=-PALF*YRPHY0%GALTAU
      PALF=MAX(YRPHY0%GALMIN,MIN(YRPHY0%GALMAX*ZRH,PALF+YRPHY2%TSPHY*(ZTALF_SCE+ZTALF_SIN)))
    ...
    ELSEIF(YRPHY0%NCVCLOS == 6) THEN
      ...
      PALF=ZS15/MAX(ZEPS16,ZS16)&
        & *MAX(0.0_JPRB,SIGN(1.0_JPRB,(ZS16-ZEPS16)))&
        & /MAX(ZTAU,ZEPS0)
      ! 2. One uses the max value between CAPE and CVGQ approaches.
      PALF=MIN(ZALFX,MAX(PALF,ZALF_CVGQ))
    ELSE
      ! Constant PALF.
      PALF=0.04_JPRB
    ENDIF
    ...
    KNND=NINT(MAX(0.0_JPRB,-SIGN(1.0_JPRB,-PALF+0.0_JPRB)))
  ...
  IF (YRPHY0%LGDDD .AND. .NOT.YRPHY0%LCVNHD) THEN
    ...
    CALL ACMTDDD( KIDIA,KFDIA,KLON,ITOP,KLEV,KTRA,&
      ...
      &INLABD,ISTPT,KSTSZ,PSTACK)
    DO JLEV=ITOP,KLEV
      
       PDDAL(JLON,JLEV)=PALF*INLABD(JLON,JLEV)*PALF/YRPHY0%FEVAPC
...
ENDDO
  ...
  DO JLEV=ITOP,KLEV
      ...
      KNLAB(JLON,JLEV)=KNND*KNLAB(JLON,JLEV)
      INBAS(JLON,JLEV)=INBAS(JLON,JLEV)*KNND
      PUDAL(JLON,JLEV)=PALF*0.5_JPRB*(ZSIGMA(JLON,JLEV)+ZSIGMA(JLON,JLEV-1))*KNLAB(JLON,JLEV)
      IF(YRPHY0%LCVNHD) PDDAL(JLON,JLEV)=YRPHY0%GUDDD*PUDAL(JLON,JLEV)
      ICDN=MAX(0,-ISIGN(1,-INCDN(JLON,JLEV)))
      PQLIC(JLON,JLEV)=PQLIC(JLON,JLEV)*KNLAB(JLON,JLEV)
      PNEBC(JLON,JLEV)=MAX(ZEPS0,ICDN*PALF&
  ...
  DO JLEV=KLEV-1,ITOP,-1
      ...
      ZFORU(JLON,JLEV)=PALF*YRPHY2%TSPHY*ZFORM(JLON,JLEV)
      ZFORM(JLON,JLEV)=MAX(0.0_JPRB,ZFORM(JLON,JLEV)-PALF/YRPHY0%FEVAPC&
        & *ZFORD(JLON,JLEV))
      ZFORM(JLON,JLEV)=PALF*YRPHY2%TSPHY*ZFORM(JLON,JLEV)
      ZFORD(JLON,JLEV)=PALF*YRPHY2%TSPHY*PALF/YRPHY0%FEVAPC*ZFORD(JLON,JLEV)
...
ZDENO = 0._JPRB
    ...
    DO JLEV=ITOP,KLEV
        ...
        ZINTE=ZBINID2(JLON,JLEV)*PDELP(JLON,JLEV)/RG
        ZUM(JLON,KLEV)=ZUM(JLON,KLEV)+PU(JLON,JLEV)*ZINTE
        ZVM(JLON,KLEV)=ZVM(JLON,KLEV)+PV(JLON,JLEV)*ZINTE
        ZDENO=ZDENO+ZINTE
      
    ENDDO
    DO JLEV=ITOP,KLEV
      
        ZUM(JLON,JLEV)=ZBINID2(JLON,JLEV)*ZUM(JLON,KLEV)/MAX(100._JPRB,ZDENO) &
          &+(1._JPRB-ZBINID2(JLON,JLEV))*PU(JLON,JLEV)
        ZVM(JLON,JLEV)=ZBINID2(JLON,JLEV)*ZVM(JLON,KLEV)/MAX(100._JPRB,ZDENO) &
          &+(1._JPRB-ZBINID2(JLON,JLEV))*PV(JLON,JLEV)
        DO JTRA=1,KTRA
          ZTRAM(JLON,JLEV,JTRA)=ZBINID2(JLON,JLEV)*ZTRAM(JLON,KLEV,JTRA)/MAX(100._JPRB,ZDENO) &
  ...
  IF(YRPHY0%LCVNAUV) THEN
    ...
    DO JLEV=KLEV-1,ITOP,-1
      ...
      ZI1(JLON,JLEV)=REAL(KNLAB(JLON,JLEV)*ZNND)
      ZNND=MIN(ZNND,1.0_JPRB-REAL(KNLAB(JLON,JLEV)))
...
ENDDO
    ...
    DO JLEV=ITOP,KLEV-1
      ...
      ZNND = MIN(ZNND,1.0_JPRB-ZI1(JLON,JLEV))
      ...
      ZDPSGDT=PDELP(JLON,JLEV)*ZGDTI*0.5_JPRB
      PSTRCU(JLON,JLEV)=ZNND*ZAUX*( PSTRCU(JLON,JLEV-1)+ &
       & ZDPSGDT*(ZUM(JLON,JLEV)+ZUM(JLON,JLEV+1)) )+ &
       & (1.0_JPRB-ZNND)*PSTRCU(JLON,JLEV-1)*ZZ
      PSTRCV(JLON,JLEV)=ZNND*ZAUX*( PSTRCV(JLON,JLEV-1)+ &
       & ZDPSGDT*(ZVM(JLON,JLEV)+ZVM(JLON,JLEV+1)) )+ &
       & (1.0_JPRB-ZNND)*PSTRCV(JLON,JLEV-1)*ZZ
      DO JTRA=1,KTRA
        PSTRCTRA(JLON,JLEV,JTRA)=ZNND*ZAUX*( PSTRCTRA(JLON,JLEV-1,JTRA)+ &
         & ZDPSGDT*(ZTRAM(JLON,JLEV,JTRA)+ZTRAM(JLON,JLEV+1,JTRA)) )+ &
         & (1.0_JPRB-ZNND)*PSTRCTRA(JLON,JLEV-1,JTRA)*ZZ
      ENDDO
  ...
  DO JLEV=ITOP,KLEV
      ...
      PFPLCL(JLON,JLEV)=KNND*PFPLCL(JLON,JLEV)
      PFPLCN(JLON,JLEV)=KNND*PFPLCN(JLON,JLEV)
      PDIFCQ(JLON,JLEV)=KNND*PDIFCQ(JLON,JLEV)
      PDIFCQL(JLON,JLEV)=KNND*PDIFCQL(JLON,JLEV)
      PDIFCQI(JLON,JLEV)=KNND*PDIFCQI(JLON,JLEV)
      PDIFCS(JLON,JLEV)=KNND*PDIFCS(JLON,JLEV)
      PDIFCTH(JLON,JLEV)=KNND*PDIFCTH(JLON,JLEV)
      PSTRCU(JLON,JLEV)=KNND*PSTRCU(JLON,JLEV)
      PSTRCV(JLON,JLEV)=KNND*PSTRCV(JLON,JLEV)
      PFCCQL(JLON,JLEV)=KNND*PFCCQL(JLON,JLEV)
      PFCCQI(JLON,JLEV)=KNND*PFCCQI(JLON,JLEV)
      PFPEVPCL(JLON,JLEV)=KNND*PFPEVPCL(JLON,JLEV)
      PFPEVPCN(JLON,JLEV)=KNND*PFPEVPCN(JLON,JLEV)
      PFHEVPP(JLON,JLEV)=KNND*PFHEVPP(JLON,JLEV)
      PFHMLTS(JLON,JLEV)=KNND*PFHMLTS(JLON,JLEV)
      DO JTRA=1,KTRA
        PSTRCTRA(JLON,JLEV,JTRA)=KNND*PSTRCTRA(JLON,JLEV,JTRA)
      ENDDO
  ...
  DO JLEV=KTDIA,KLEV
      ...
      KNLAB(JLON,JLEV)=KNND*KNLAB(JLON,JLEV)
      PQLIC(JLON,JLEV)=KNND*PQLIC(JLON,JLEV)*YRPHY0%FQLIC*PNEBC(JLON,JLEV)*KNLAB(JLON,JLEV)
      PNEBC(JLON,JLEV)=KNND*MAX(ZEPS0,MIN(ZNEBCX,PNEBC(JLON,JLEV)*YRPHY0%FNEBC&
...
!DEC$ IVDEP
      ...
      PUDAL(JLON,JLEV)=KNND*PUDAL(JLON,JLEV)
      PDDAL(JLON,JLEV)=KNND*PDDAL(JLON,JLEV)
      ...
      PENTR_U(JLON,JLEV)=ZWU(JLON,JLEV)*PUDAL(JLON,JLEV)*ZEPSILON_U(JLON,JLEV)*IQLIC
      ...
      PENTR_D(JLON,JLEV)=-ZWD(JLON,JLEV)*PDDAL(JLON,JLEV) &
         ...
         &/(PR(JLON,JLEV)*ZT(JLON,JLEV))+ZENTRX)*RG*IQLIC*YRPHY0%GCVFRR
...
IF (YRPHY0%NCAPEMOD /= 0) THEN
  ...
  CALL FPCINCAPE(KIDIA,KFDIA,KLON,KLEV,KLEV, &
      & PT,PAPRSF,PQ,PCAPE,ZCIN,ILCL,IFCL,IEL,ISTPT,KSTSZ,PSTACK)
  !
  !- - - - - - - - - - - - - - - - - - - - - - -
