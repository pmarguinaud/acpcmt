...
REAL(KIND=JPRB) :: ZAUTOL(KLON,KLEV,KGPBLKS), ZAUTOI(KLON,KLEV,KGPBLKS) &
...
! -----------------------------------------------------------
    ZEFFA(JLON,JLEV,JBLK) = EXP(YRPHY0%RAUTSBET*ZDELT(JLON,JLEV,JBLK))
...
! -----------------------
    ...
    ZQL = MAX(0.0_JPRB,PQL(JLON,JLEV,JBLK)/PNEBST(JLON,JLEV,JBLK))
    ZQI = MAX(0.0_JPRB,PQI(JLON,JLEV,JBLK)/PNEBST(JLON,JLEV,JBLK))
...
! ----------------------------------------------
    ...
    ZAUTOL(JLON,JLEV,JBLK) = MAX(0.0_JPRB,ZDUM)

! AUTOCONVERSION OF CLOUD ICE INTO PRECIPITATING ICE
! --------------------------------------------------

    ZCAUT = YRPHY0%RAUTEFS * ZEFFA(JLON,JLEV,JBLK)
    ZQCR = YRPHY0%RQICRMAX - (YRPHY0%RQICRMAX - YRPHY0%RQICRMIN) * 0.5_JPRB &
     & * (1.0_JPRB + TANH(ZALPH * ZDELT(JLON,JLEV,JBLK) + ZBETA))
    ZFACICE = PLSM(JLON,JBLK)*PNEIJ(JLON,JBLK) + (1.0_JPRB-PLSM(JLON,JBLK)) &
     & * MAX(0.0_JPRB,SIGN(1.0_JPRB,YRPHY1%TMERGL-PTS(JLON,JBLK)))
    ZQCR = ZQCR * (1.0_JPRB-ZFACICE*(1.0_JPRB-YRPHY0%RQICRSN))
    ZDUM = (1.0_JPRB-EXP(-ZCAUT*YRPHY2%TSPHY)) * (ZQI-ZQCR) / YRPHY2%TSPHY
    ZAUTOI(JLON,JLEV,JBLK) = MAX(0.0_JPRB,ZDUM)
 
! TOTAL AUTOCONVERSION TERM
! -------------------------

    PAUTOL(JLON,JLEV,JBLK) = ZAUTOL(JLON,JLEV,JBLK) * PNEBST(JLON,JLEV,JBLK)
    PAUTOI(JLON,JLEV,JBLK) = ZAUTOI(JLON,JLEV,JBLK) * PNEBST(JLON,JLEV,JBLK)
