...
SUBROUTINE ACNEBXRS ( KIDIA,KFDIA,KLON,KTDIA,KLEV,&
 ...
 & PQ,PQC,PQSAT,&
 ! - OUTPUT 2D .
 & PNEB, KSTACK, PSTACK, KPSTSZ, KKSTSZ, KPSTPT, KKSTPT)  
...
IF (YRPHY%LQXRTGH) THEN
  DO JLEV=KTDIA,KLEV
    ...
    &  EXP(-YRPHY0%QXRAL*PQC(JLON,JLEV)/((1.0_JPRB-ZRHLIM)*PQSAT(JLON,JLEV))**YRPHY0%QXRDEL))
...
ELSE
  DO JLEV=KTDIA,KLEV
    DO JLON=KIDIA,KFDIA
      ZRH=MIN(YRPHY0%QXRHX,PQ(JLON,JLEV)/PQSAT(JLON,JLEV))
      ZRHLIM=MAX(ZEPS1,MIN(1.0_JPRB-ZEPS1,ZRH))
      ZNEB=ZRHLIM**YRPHY0%QXRR*(1.0_JPRB-&
    &  EXP(-YRPHY0%QXRAL*PQC(JLON,JLEV)/((1.0_JPRB-ZRHLIM)*PQSAT(JLON,JLEV))**YRPHY0%QXRDEL))
      ZBIN=MAX(0.0_JPRB,SIGN(1.0_JPRB,ZRH-1.0_JPRB))
